...
void QRoundProgressBar::drawValue(QPainter &p, const QRectF &baseRect, double value, double arcLength, const QRectF & innerRect, double innerRadius)
{
    // nothing to draw
    if (value == m_min)
        return;
    ...
    // pie segment outer
    dataPath.moveTo(baseRect.center());
    dataPath.arcTo(baseRect, m_nullPosition, -arcLength); //  绘制外圆的弧长
	...
	if (m_barStyle == StyleDonut)
	{
		...
		currentPoint = baseRect.center() 
			+ ((currentPoint - baseRect.center()) * m_innerOuterRate); //计算内圆的坐标点，m_innerOuterRate替代了原作者写的0.75，代表内圆是外圆的0.75倍
